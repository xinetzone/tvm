...
def find_libvta(lib_vta, optional=False):
    ...
    optional : bool
        Enable error check
    """
    curr_path = Path(__file__).expanduser().resolve().parent
    tvm_library_path = os.environ.get("TVM_LIBRARY_PATH", None)
    if tvm_library_path is None:
       tvm_library_path = curr_path.parent.parent.parent / "build" / "vta"
    lib_search = [tvm_library_path, Path(get_vta_hw_path()) / "build/vta"]
    lib_name = _get_lib_name(lib_vta)
    lib_path = [str(x / lib_name) for x in lib_search]
    lib_found = [str(x) for x in lib_path if Path(x).exists()]
