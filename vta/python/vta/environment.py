...
from tvm import te
from . import intrin
from .target import vta_target


def get_vta_hw_path():
    """Get the VTA HW path."""
    curr_path = Path(__file__).expanduser().resolve().parents[2]
    vta_hw_default = (curr_path / "vta_hw").resolve()
    VTA_HW_PATH = os.getenv("VTA_HW_PATH", vta_hw_default)
    return Path(VTA_HW_PATH).resolve()


def pkg_config(cfg):
    """Returns PkgConfig pkg config object."""
    pkg_config_py = Path(get_vta_hw_path()) / "config/pkg_config.py"
...
class Environment(object):
    ...
    @property
    def target(self):
        return vta_target(model=self.MODEL)
