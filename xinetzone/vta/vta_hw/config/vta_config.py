...
def pkg_config(cfg):
    ...
    pkg_config_py = Path(__file__).expanduser().resolve().parent / "pkg_config.py"
...
def main():
    ...
    path_list = [
        "vta_config.json", curr_path / "vta_config.json"
    ]

    if args.use_cfg:
        path_list = [args.use_cfg]

    ok_path_list = [p for p in path_list if Path(p).exists()]
    if not ok_path_list:
