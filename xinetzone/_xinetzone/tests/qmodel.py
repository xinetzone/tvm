...
class TorchModel:
    ...
    @property
    def model(self):
        mod = getattr(qmodels, self.model_name)
    ...
    def scripted_model(self,
        ...
        model = self.model
        if fuse_model:
            fuse_module(model, backend='fbgemm')
            # print(model)
...
class RelayFunction:
    ...
    def topk(runtime, val_loader, print_freq=100):
        ...
        for k, (xs, ys) in enumerate(val_loader):
            ...
            _, topk_indices = find_topk(output.numpy(), k=5)
            ys = ys.numpy()
    ...
    @staticmethod
    def float_topk(val_loader, model_name='resnet18',
                   quantize=False,
                   pretrained=True, print_freq=100, **kwargs):
        test_nums = len(val_loader)
