...
def find_width8(op):
    ...
    if (
        isinstance(op, tvm.tir.For)
        and isinstance(op.extent, tvm.tir.IntImm)
        and op.extent.value % 8 == 0
    ):
        loops.append(op)
...
def vectorize8(op):
    ...
    if op in loops:
        ...
        lo, li = te.var(f"{name}.outer"), te.var(f"{name}.inner")
    ...
    return (
        f.with_body(
            tvm.tir.stmt_functor.ir_transform(
                f.body, None, vectorize8, ["tir.For"]
            )
        )
        if loops
        else f
